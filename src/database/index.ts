...
export function getDatabaseManagerClass(config: Config): BaseDatabaseManager<BaseRow, BaseTable<BaseRow>> {
    switch (config.DATABASE_PLATFORM) {
        ...
        case 'sqlite':
            return new SqliteDatabaseManager(config.DATABASE_NAME!);
        default:
