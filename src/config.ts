...
type ConfigTypeConstructor = StringConstructor | NumberConstructor | BooleanConstructor;
...
type ConfigPropertyRecord = Record<string, ConfigProperty<ConfigTypeConstructor, boolean>>;

/** Configuration property definitions */
export const ConfigProperties = {
    DEBUG_MODE: { type: Boolean, envVar: 'DEBUG_MODE', required: true, priority: ConfigPriority.CORE },

    // Authentication
    AUTH_ENABLED: { type: Boolean, envVar: 'AUTH_ENABLED', required: true, priority: ConfigPriority.SYSTEM },
    AUTH_KEY: {
        type: String,
        envVar: 'AUTH_KEY',
        required: false,
    ...
    // HTTP
    HOST: { type: String, envVar: 'APP_HOST', required: false, priority: ConfigPriority.SERVICE },
    PORT: { type: Number, envVar: 'APP_PORT', required: true, priority: ConfigPriority.SYSTEM },
    IDLE_TIMEOUT: { type: Number, envVar: 'IDLE_TIMEOUT', required: false, priority: ConfigPriority.SERVICE },
    MAX_REQUEST_BODY_SIZE: { type: Number, envVar: 'MAX_REQUEST_BODY_SIZE', required: false, priority: ConfigPriority.SERVICE },

    // TLS
    TLS_ENABLED: { type: Boolean, envVar: 'TLS_ENABLED', required: true, priority: ConfigPriority.SYSTEM },
    TLS_CERT_PATH: {
        type: String,
...
export function loadConfig(): Config {
    ...
    const entries = Object.entries(ConfigProperties).sort(
        ([, a], [, b]) => (a.priority ?? ConfigPriority.LOW) - (b.priority ?? ConfigPriority.LOW)
    );
    ...
    for (const [key, prop] of entries) {
        ...
        if (envValue === undefined) {
            const conditionallyRequired = 'requiredIf' in prop ? prop.requiredIf(config) ?? false : false;
